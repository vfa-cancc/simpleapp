...
public protocol SwipeCellDelegate {
    func swipeableTableViewCellDidStartSwiping(_ cell: SwipeCell)
    func swipeableTableViewCellDidEndSwiping(_ cell: SwipeCell)
    func swipeableTableViewCell(_ cell: SwipeCell, didSwipeWithPercentage percentage: CGFloat)
}

open class SwipeCell: UITableViewCell {
    
    fileprivate typealias `Self` = SwipeCell
    ...
    public typealias MSSwipeCompletionBlock = (SwipeCell, SwipeCellState, SwipeCellMode) -> ()
    
    public var delegate: SwipeCellDelegate?
    var direction: SwipeCellDirection!
    ...
    var modeForState1: SwipeCellMode!
    var modeForState2: SwipeCellMode!
    var modeForState3: SwipeCellMode!
    var modeForState4: SwipeCellMode!
    var color1: UIColor!
    var color2: UIColor!
    var color3: UIColor!
    var color4: UIColor!
    ...
    func initializer() {
        ...
        panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(SwipeCell.handlePanGestureRecognizer(_:)))
    ...
    public func setSwipeGesture(_ view: UIView, color: UIColor, mode: SwipeCellMode, state: SwipeCellState, completionHandler: @escaping MSSwipeCompletionBlock) {
    ...
    @objc func handlePanGestureRecognizer(_ gesture: UIPanGestureRecognizer) {
        ...
        if (state == .began || state == .changed) {
            ...
            var cellMode: SwipeCellMode = .none
    ...
    func directionWithPercentage(_ percentage: CGFloat) -> SwipeCellDirection {
        if percentage < 0 {
            return .left
    ...
    func stateWithPercentage(_ percentage: CGFloat) -> SwipeCellState {
    ...
    func moveWithDuration(_ duration: TimeInterval, direction: SwipeCellDirection) {
        isExited = true
    ...
    func executeCompletionBlock() {
        let state: SwipeCellState = stateWithPercentage(currentPercentage)
        var mode: SwipeCellMode = .none
