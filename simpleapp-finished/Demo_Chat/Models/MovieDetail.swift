...
struct MovieDetail : MovieAppModel {
    ...
    var countries: [ProductionCountries] = [ProductionCountries]()
    var budget: Double = 0
    ...
    init?(jsonData: JSONData) {
        ...
        if let poster_path = jsonData["poster_path"] as? String, poster_path != "" {
            self.poster_path = "\(IMAGE_API)\(POSTER_SIZE_KEY)\(poster_path)"
        }
        ...
        if let countriesDataList = jsonData["production_countries"] as? [JSONData], countriesDataList.count > 0 {
            for countriesData in countriesDataList {
                if let countries = ProductionCountries(jsonData: countriesData) {
        ...
        if let backdrop_path = jsonData["backdrop_path"] as? String, backdrop_path != "" {
            self.backdrop_path = "\(IMAGE_API)\(BACKDROP_SIZE_KEY)\(backdrop_path)"
        }
